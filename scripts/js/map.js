function initialize() {
  ...
  geocoder.geocode({'address': 'Clinton - WA'}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      ...
      var infowindowclintonwa = new google.maps.InfoWindow({
        content: '<h1 class="post-title">Clinton, WA</h1><ul><li><a href="/products/mason-style-storage-jar-with-wood-lid">Mason Style Storage Jar with Wood Lid</a></li></ul><p><a href="/products/in/clinton-wa" class="view-all">View all</a></p>'
      });
      google.maps.event.addListener(marker, 'click', function() {
        for (var i=0;i<infowindows.length;i++) {
          infowindows[i].close();
        }
        infowindowclintonwa.open(map,marker);
      });
      infowindows.push(infowindowclintonwa);
    }
  });
